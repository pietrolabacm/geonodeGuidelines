...
acesse o menu "Editar-Carregar Estilo" (Figura \ref{fig:importarEstilo}.1) e na
nova página arraste e solte o arquivo SLD ou encontre-o pelo explorador de
arquivos no botão "Escolher Arquivos" (Figura \ref{fig:importarEstilo}.2). Por
fim clique no botão de upload (Figura \ref{fig:importarEstilo}.3).

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/importarEstilo.pdf}
  \caption{Fluxo para a importação de estilo de um arquivo SLD.}
  \label{fig:importarEstilo}
\end{figure}
...
\textbf{Atenção, caso o campo de atributo utilizado nas regras de simbologia
contenha letras maiúsculas, ele deve ser usado no sld apenas com letras
minúsculas. Isso ocorre por conta dos padrões de nomenclatura do Geonode
acompanharem os do PostgreSQL que utiliza os nome dos campos em letras
minúsculas. Utilizar uma regra SLD com um campo com letras maiúsculas irá
impedir que a camada seja renderizada.}
...
\textbf{Para a filtragem dos dados é importante que os metadados
 ...
 seu sítio de pesquisa como Palavra-chave.}
