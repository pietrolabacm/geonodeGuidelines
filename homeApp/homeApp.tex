...
\subsection{Tipos de dados}\label{sec:tiposDeDados}
...
\begin{description}
  ...
  \item[display\_name]
    - Um objeto da classe 
      ...
      {CharField}} do Django que gera uma coluna do tipo VARCHAR na tabela do
      banco de dados e possui um limite de 50 caracteres. Este nome será
      exposto como o título do subsite na homepage do Geonode e é um atributo
      obrigatório.
...
\begin{description}
  ...
  \textcolor{blue}
    ...
    {ForeignKey}} do Django que gera uma coluna de relacionamento 1:N
    correlacionando a categoria com o índice de seu respectivo subsite.
  ...
  - Um objeto da classe
    ...
    {ManyToManyField}} do Django que gera uma coluna de relacionamento N:N que
    correlaciona por meio de uma tabela intermediária uma categoria com um
    grupo de keywords que podem ser utilizados como filtro. Esses keywords são
    do modelo de 
    \textcolor{blue}
    {\href{https://docs.geonode.org/en/master/admin/admin_panel/index.html#hierarchical-keywords}
    {HierarchicalKeywords}} do Geonode. Atributo obrigatório.

  \item[keywords\_filter\_url]
  - Um objeto da classe 
    \textcolor{blue}
    {\href{https://docs.djangoproject.com/en/5.0/ref/models/fields/#django.db.models.CharField}
    {CharField}} do Django que gera uma coluna do tipo VARCHAR com um limite de
    ...
    interface gráfica do Django Admin. Seu propósito é armazenar os parâmetros
    de filtragem definidos pela coluna keyword.
\end{description}
...
ícone do perfil do usuário e no submenu Admin (Figura
\Ref{fig:acessoExtendedSubSite}.1). Os cadastros serão feitos sob os membros do
grupo do aplicativo Home: Extended Subsites e Categories.

\subsection{Extended Subsites}

Na página de administração dos Extended Subsites é possível visualizar, editar,
criar e excluir os subsites cadastrados. Para visualizar ou editar um subsite
basta clicar em seu nome e para excluí-lo selecione sua caixa e no menu
drop-down no canto inferior esquerdo selecione excluir e confirme. Para criar
um novo subsite clique no botão "adicionar extended sub site" no canto superior
direito (Figura \Ref{fig:acessoExtendedSubSite}.3). 

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/acessoExtendedSubSites.pdf}
  \caption{Acesso à administração do subsites na interface de administrador do geonode e acessando a página de criação de um novo subsite.}\label{fig:acessoExtendedSubSite}
\end{figure}

Uma vez na tela de criação de subsite basta preencher os campos dos atributos
com os dados apropriados (Figura \Ref{fig:criarExtendedSubSites}), mais
informações sobre estes campos podem ser encontradas na
\hyperref[sec:tiposDeDados]{seção Tipos de Dados}. Nos campos de filtro, caso
nenhum atributo para filtro seja selecionado todos os elementos serão
considerados, se um ou mais atributos forem selecionados, apenas recursos com
aqueles atributos serão mostrados no subsite. Por fim salve as alterações
usando o botão "Salvar" no canto inferior direito.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/criarExtendedSubSites.pdf}
  \caption{Tela de criação de subsites.}\label{fig:criarExtendedSubSites}
\end{figure}
...
\begin{description}
  ...
  \item[Site name] - Nome do subsite no formato slug (nome formatado para uso em URL, exemplo: nome-do-site).
  
  \item[Display Name] - Nome exibido nos cartões na página inicial.
  
  \item[Display Description] - Descrição exibida nos cartões na página inicial.
  
  \item[Display image] - Imagem exibida nos cartões na página inicial.
  
  \item[Theme] - Tema a ser utilizado pelo subsite. Deve ser escolhido entre os temas disponíveis ou mantido vazio para usar o tema padrão do Geonode.
  
  \item[Logo] - Imagem de logo do subsite, que substitui o logo usado no tema \textcolor{red}{CONFIRMAR}.
  
  \item[Subsite description] - Descrição do subsite, visível apenas no menu de administração.
  
  \item[Types] - Tipos de dados contidos no subsite.
  
  \item[Region] - Regiões classificadas contidas no subsite.
  
  \item[Category] - Categorias de dados contidas no subsite.
  
  \item[Keyword] - Palavras-chave associadas aos dados contidos no subsite.
  
  \item[Group] - Grupos que podem ter seus dados disponibilizados no subsite.
  
  \item[Allowed permissions] - Permissões para os usuários que acessam o site. Se nenhuma permissão for selecionada, os usuários terão, por padrão, acesso à visualização e ao download.
  \end{description}

\subsection{Categories}

Semelhante ao Extended Subsites, na página "Categories", o usuário pode
visualizar, editar, excluir e adicionar novas categorias à página inicial. O
processo de criação é semelhante ao dos subsites com exceção de que um número
reduzido de campos deve ser preenchido (Figura \Ref{fig:criarCategoria}). Os
demais campos, que não estão presentes na criação da categoria, são herdados do
subsite no qual a categoria está inserida.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{img/criarCategoria.pdf}
  \caption{Tela de criação de categorias.}\label{fig:criarCategoria}
\end{figure}
...
\begin{description}
  ...
  \item[Subsite] - Referência ao subsite o qual a categoria está atrelada.
  \item[Display Name] - Nome exibido nos cartões da página inicial.
  \item[Display Description] - Descrição exibida nos cartões da página inicial.
  \item[Display image] - Imagem exibida nos cartões da página inicial.
  \item[Keyword] - Palavras-chave dos dados contidas na categoria.
