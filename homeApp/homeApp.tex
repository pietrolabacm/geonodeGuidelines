...
drop-down no canto inferior esquerdo selecione excluir e confirme. Para criar um novo subsite clique no botão "adicionar extended sub site" no
canto superior direito (Figura \Ref{fig:acessoExtendedSubSite}.3). 
...
com os dados apropriados, mais informações sobre estes campos podem ser
encontradas na \hyperref[sec:tiposDeDados]{seção Tipos de Dados}. Nos campos de
filtro, caso nenhum atributo para filtro seja selecionado todos os elementos
serão considerados, se um ou mais atributos forem selecionados, apenas recursos
com aqueles atributos serão mostrados no subsite. Por fim salve as alterações
...
\begin{description}
  ...
  \item[Site name] - Nome do sub site em forma de slug (nome em formatação apropriada para a url, exemplo: nome-do-site).
  \item[Display Name] - Nome que será apresentado nos cartões da página inicial
  \item[Display Description] - Descrição que será apresentada nos cartões da página inicial
  \item[Display image] - Imagem que será apresentada nos cartões da página inicial
  \item[Theme] - Tema a ser utilizado pelo subsite. Deve ser selecionado entre os temas existentes, ou mantido vazio para usar o padrão do Geonode.  
  \item[Logo] - Imagem de logo do subsite, sobrescreve o logo usado no tema \textcolor{red}{CONFIRMAR}.
  \item[Subsite description] - Descrição do subsite, só pode ser visualizado dentro do menu de administração.
  \item[Types] - Tipos de dado contidos no subsite.
  \item[Region] - Regiões classificadas contidas no subsite.
  \item[Categoria] - Categorias de dados contidas no subsite.
  \item[Keyword] - Palavras-chave dos dados contidas no subsite.
  \item[Grupo] - Grupos que podem ter seus dados disponibilizados no subsite.
  \item[Allowed permissions] - Permissões para os usuários que acessam o site. Se nenhuma permissão for selecionada, os usuários terão acesso a visualização e download por padrão. 
\end{description}
